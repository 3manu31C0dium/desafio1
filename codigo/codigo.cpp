...
String identificar_forma_onda() {
    ...
    int cambios_bruscos = 0;
    float umbral_cambio_brusco = 0.5; // Ajustar según la amplitud típica de la señal
    float ultima_muestra = muestras[0];
    
    for (int i = 1; i < TAMANO_BUFFER; i++) {
        float derivada = abs(muestras[i] - ultima_muestra);
        suma_derivadas += derivada;
        
        if (derivada > umbral_cambio_brusco) {
            cambios_bruscos++;
        }
        
        ultima_muestra = muestras[i];
    }
    
    float promedio_derivadas = suma_derivadas / (TAMANO_BUFFER - 1);
    
    if (cambios_bruscos > TAMANO_BUFFER / 50) { // Si hay muchos cambios bruscos, es probablemente una onda cuadrada
        return "Cuadrada";
    } else if (promedio_derivadas < 0.1) { // Si las derivadas son muy pequeñas, es probablemente una onda triangular
        return "Triangular";
    } else {
        return "Senoidal"; // Si no es ni cuadrada ni triangular, asumimos que es senoidal
    }
}
